...
__global__ void allocateWorld(hittable **d_list, hittable **d_world, camera **d_cam) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        *(d_cam) = new camera();
        ...
        material *matCenter = new lambertian((color(0.7f, 0.3f, 0.3f)));
        material *matLeft = new metal(color(0.8f, 0.8f, 0.8f), 0.3f);
        material *matRight = new metal(color(0.8f, 0.6f, 0.2f), 1.0f);
        ...
        *(d_list+3) = new sphere(vec3(1.0f, 0.0f, -1.0f), 0.5f, matRight);
    }
}
...
int main(void) {
    ...
    hittable **d_list;
    cudaStatus = cudaMalloc((void**)&d_list, 4 * sizeof(hittable*));
    checkReturn(cudaStatus);
    ...
    // Output FB as Image
    std::ofstream ppmFile("metal_spheres.ppm");
