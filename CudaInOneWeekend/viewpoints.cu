...
__global__ void allocateWorld(hittable **d_list, hittable **d_world, camera **d_cam) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        *(d_world) = new hittable_list(d_list, 5);
        ...
        material *matCenter = new lambertian(color(0.1f, 0.2f, 0.5f));
        material *matLeft = new dielectric(1.5f);
        material *matRight = new metal(color(0.8f, 0.6f, 0.2f), 0.0f);

        // Spheres
        *(d_list) = new sphere(vec3(0.0f, -100.5f, -1.0f), 100.0f, matGround);
        *(d_list+1) = new sphere(vec3(0.0f, 0.0f, -1.0f), 0.5f, matCenter);
        *(d_list+2) = new sphere(vec3(-1.0f, 0.0f, -1.0f), 0.5f, matLeft);
        *(d_list+3) = new sphere(vec3(-1.0f, 0.0f, -1.0f), -0.45f, matLeft);
        *(d_list+4) = new sphere(vec3(1.0f, 0.0f, -1.0f), 0.5f, matRight);

        // Camera
        *d_cam = new camera(point3(-2.0f,2.0f,1.0f), point3(0.0f,0.0f,-1.0f), vec3(0.0f,1.0f,0.0f), 90.0f, 2.0f);
...
int main(void) {
    ...
    hittable **d_list;
    cudaStatus = cudaMalloc((void**)&d_list, 5 * sizeof(hittable*));
    checkReturn(cudaStatus);
    ...
    // Output FB as Image
    std::ofstream ppmFile("viewpoints.ppm");
