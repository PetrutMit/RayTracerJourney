...
__global__ void allocateWorld(hittable **d_list, hittable **d_world, camera **d_cam, curandState *randState) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        *(d_list + 6) = new box(vec3(130.0f, 0.0f, 65.0f), vec3(295.0f, 165.0f, 230.0f), white);
        *(d_list + 7) = new box(vec3(265.0f, 0.0f, 295.0f), vec3(430.0f, 330.0f, 460.0f), white);
...
int main(void) {
    ...
    // Output FB as Image
    std::ofstream ppmFile("cornell_box_with_boxes.ppm");
