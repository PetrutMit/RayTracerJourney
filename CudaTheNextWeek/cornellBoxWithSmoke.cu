...
__global__ void allocateWorld(hittable **d_list, hittable **d_world, camera **d_cam, curandState *randState) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        *(d_list + 5) = new xy_rect(0.0f, 555.0f, 0.0f, 555.0f, 555.0f, white);
        ...
        box1 = new translate(box1, vec3(0.0f, 0.0f, 0.0f));
        box1 = new constant_medium(box1, 0.01f, new constant_texture(vec3(0.0f, 0.0f, 0.0f)), randState);

        hittable *box2 = new box(vec3(0.0f, 0.0f, 0.0f), vec3(165.0f, 165.0f, 165.0f), white);
        //box2 = new rotate_y(box2, -18.0f);
        box2 = new translate(box2, vec3(0.0f, 0.0f, 0.0f));
        box2 = new constant_medium(box2, 0.01f, new constant_texture(vec3(1.0f, 1.0f, 1.0f)), randState);
...
int main(void) {
    ...
    // Output FB as Image
    std::ofstream ppmFile("cornell_box_smoke.ppm");
